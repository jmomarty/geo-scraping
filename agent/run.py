...
DEFAULT_OUTPUT_PATH = Path("data/chatgpt_answers.jsonl")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Open chatgpt.com in a logged-out browser, submit a prompt, "
            "extract the answer, and append it to JSONL output."
        )
    )
    parser.add_argument("--prompt", required=True, help="Prompt text to send to ChatGPT")
    parser.add_argument(
        "--output",
...
def dismiss_entry_popup(page: "Page") -> bool:
    for selector in ENTRY_POPUP_DISMISS_SELECTORS:
        button = page.locator(selector).first
        try:
...
def wait_for_prompt_input(page: "Page", timeout_seconds: int) -> "Locator":
    ...
    raise TimeoutError(
        "Timed out waiting for ChatGPT prompt input. "
        "If a verification screen is shown, solve it manually and retry."
    )
...
def wait_for_final_answer(
    ...
    while time.monotonic() < deadline:
        ...
        if ui_error:
            raise RuntimeError(f"chatgpt_ui_error: {ui_error}")
        ...
        if not saw_meaningful_text and now >= stall_deadline and (generating or has_assistant_turn(page)):
            raise TimeoutError(
                "stalled_generation: ChatGPT remained in-progress without meaningful "
...
def run(
    ...
    run_id = f"chatgpt-{datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%S')}-{uuid4().hex[:8]}"
    summary = {
        "provider": "chatgpt",
        "run_id": run_id,
        ...
        with sync_playwright() as playwright:
            ...
            for sample_index in range(1, samples + 1):
                ...
                try:
                    ...
                    if "has been closed" in str(sample_exc).lower() and not page.is_closed():
                        dismiss_entry_popup(page)
